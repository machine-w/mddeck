...
import { isEnabledAutoScaling } from './utils.js'
...
export const fittingHeaderPlugin = marpPlugin((md) => {
  ...
  md.core.ruler.after('inline', 'marp_fitting_header', ({ tokens }) => {
    // mddeck: fitting-header is useful even without inlineSVG, so we don't
    // check inlineSVG here (unlike marp-core). The custom element it emits
    // is still defined by scaffold CSS or theme.
