...
import twemojiCSS from './twemoji.js'
...
export const markdown = marpPlugin((md) => {
  const opts: EmojiOptions =
    (md.marpit as any)._mddeckEmojiOption ??
    md.marpit.options?.emoji ??
    ({ shortcode: 'twemoji', unicode: 'twemoji', twemoji: {} } as EmojiOptions)
  ...
  if (opts.shortcode) {
    ...
    md.core.ruler.push('marp_emoji', (state: any) => {
      const { Token } = state

      state.Token = function replacedToken(this: unknown, type: string, tag: string, nesting: unknown) {
        return new Token(type === 'emoji' ? 'marp_emoji' : type, tag, nesting)
      } as unknown as typeof Token

      ;(picker as any).rule(state)
      state.Token = Token
    })
        ...
        if (token.type === 'inline') {
          ...
          for (const t of token.children ?? []) {
            if (t.type === 'text') {
              ...
              const reduced: Token[] = []
              for (let idx = 0; idx < splittedByEmoji.length; idx++) {
                const text = splittedByEmoji[idx]
