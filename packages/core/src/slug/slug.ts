...
export const markdown = marpPlugin((md) => {
  ...
  md.core.ruler.push('marp_slug', (state) => {
    // marp-core adds a custom `slug` option; we keep it on the marpit options bag.
    const opts = parseSlugOptions((marp as any)._mddeckSlugOption ?? (marp as any).options?.slug ?? true)
    if (!opts) return
